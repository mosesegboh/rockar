<template>
    <div id="car-finder">
        <div class="general-preloader" v-show="ajaxLoading || CarFilter.ajaxLoading"><div class="show-loading"></div>
        </div>
        ...
        <div class="navigation-buttons-wrapper col-12"
             ...
             :class="{'extra-small-window': extraSmallWindow, 'footer-margin': currentStep === 3 &&
             this.CarFilter.productCount === 1,'px-second-step': partExchangeSecondStep}"
            ...
            <div class="navigation-outer-container">
                ...
                <button class="button button-narrow next-step desktop-tablet-only"
                    ...
                    v-show="!partExchangeLastStep && !partExchangeSecondStep && currentStep !== steps.length - 1 &&
                    currentStep !== 2"
                ...
                <button class="button button-narrow next-step continue desktop-tablet-only"
                    ...
                    @click="nextStep(false)"
                >
                    {{ modelButtonTitle | translate }}
                </button>

                <!-- Mobile navigation global button -->
                <button class="button button-narrow next-step mobile-only"
                    :class="{'with-back': currentStep === 1 || currentStep === 2, 'full-width': currentStep === 0,
                    'disabled': disabledNext}"
                    :disabled="disabledNext"
                    v-show="!partExchangeSecondStep && !partExchangeLastStep"
                    @click="nextStep(false)"
                >
                    {{ modelButtonTitle | translate }}
                </button>
                ...
                <button class="button button-wide button-narrow next-step continue-px with-px"
                    :class="[!hasNegativeEquity || isPayInFull ? 'button-default' : 'button-disabled']"
                    :disabled="hasNegativeEquity && !isPayInFull"
                    @click="updateOutstandingFinance() + nextStep(false)"
                    v-if="partExchangeLastStep"
                >
                    {{ 'Next' | translate }}
                </button>
                <button class="button button-narrow button-gray-light-2 button-wide next-step continue-px without-px
                    text-center desktop-tablet-only"
                ...
                <button class="button button-narrow step-back mobile-only"
                    v-show="currentStep === 1 && (PartExchangeFilter.stepTwoSubStep <= 2) || currentStep === 2 ||
                    currentStep === 3"
                    @click="prevStep"
                >
                    <span></span>
        ...
        <app-modal :show.sync="editPartExchange" v-show="editPartExchange" class="carfinder-px-modal">
            <div slot="content" class="car-finder-px-modal">
                <app-configurator-part-exchange
                    ...
                    :outstanding-finance-settlement="0"
                ></app-configurator-part-exchange>
            </div>
        </app-modal>
    </div>
...
<script>
    ...
    import appNavigationSteps from 'bmw/components/CarFinder/NavigationSteps';
    ...
    export default Vue.extend({
        ...
        watch: {
            editPartExchange(val) {
                if (!val) { // On modal close
                    if (this.ConfiguratorPartExchange.checkData() &&
                        this.ConfiguratorPartExchange.PXValuation.valuationCompleted) {
                        ...
                        this.PartExchange.PXVrm.carInfo.registrationYear = this.ConfiguratorPartExchange.PXVrm.carInfo.registrationYear;

                        this.PartExchange.PXVrm.carDetails.result =
                            this.ConfiguratorPartExchange.PXVrm.carDetails.result;
                        this.PartExchange.PXVrm.carDetails.vrmDisabled =
                            this.ConfiguratorPartExchange.PXVrm.carDetails.vrmDisabled;
                        ...
                        this.PartExchange.PXValuation.valuationResult =
                            this.ConfiguratorPartExchange.PXValuation.valuationResult;
                        this.PartExchange.PXValuation.partExchangeValue =
                            this.ConfiguratorPartExchange.PXValuation.partExchangeValue;
                        this.PartExchange.PXValuation.valuationCompleted =
                            this.ConfiguratorPartExchange.PXValuation.valuationCompleted;
                        this.PartExchange.PXValuation.outstandingFinance =
                            this.ConfiguratorPartExchange.PXValuation.outstandingFinance;
                    ...
                    this.$nextTick(() => {
                        ...
                        this.ConfiguratorPartExchange.PXVrm.carInfo.registrationYear = null;
                        ...
                        if (this.PartExchange.PXVrm.carInfo.vrmInput ||
                            this.PartExchange.PXVrm.carInfo.vrmInput.length) {
                            this.ConfiguratorPartExchange.PXVrm.carInfo.vrm = this.PartExchange.PXVrm.carInfo.vrm;
                            this.ConfiguratorPartExchange.PXVrm.carInfo.vrmInput =
                                this.PartExchange.PXVrm.carInfo.vrmInput;
                        }
                        ...
                        if (this.PartExchange.activeCondition) {
                            ...
                            this.ConfiguratorPartExchange.activeConditionSelectIndex =
                                this.PartExchange.activeConditionSelectIndex;
                            if (typeof this.ConfiguratorPartExchange.$refs.conditionSlider !== 'undefined') {
                                this.ConfiguratorPartExchange.$refs.conditionSlider.changeActive(
                                    this.PartExchange.activeCondition);
                            }
                        }
        ...
        computed: {
            ...
            hasNegativeEquity() {
                return this.PartExchange.PXValuation.outstandingFinance > (
                    this.PartExchange.PXValuation.partExchangeValue +
            ...
            'CarFinder::resetFilters'() {
                ...
                this.FinanceFilter.hardResetFilters(() => {
                    ...
                    this.PartExchangeFilter.hardResetPartExchange(() => {
                        ...
                        this.$store.commit('setNotificationMessage', { message:
                                this.translateString('All filters are reset.'), type: 'success', timeout: 5000 });
                    });
                });
        ...
        methods: {
            ...
            updateButtonTitle() {
                switch (this.currentStep) {
                    case 2:
                        this.modelButtonTitle = `Show ${this.ModelFilter.activeItemsCount} results`;
                        break;
                    default:
                        this.modelButtonTitle = 'Next step';
                        break;
                }
            },
                ...
                if (!passed) {
                    switch (this.currentStep) {
                        ...
                        case 2: // Model filter
                            ...
                            this.updateCurrentStep();
                            this.CarFilter.updateFilters();
                            this.$broadcast('PartExchangeFilterMenu::updateSelectedCondition');
                            break;
                ...
                switch (this.currentStep) {
                    ...
                    case 2:
                        if (this.PartExchangeFilter.stepTwoSubStep > 1) {
                             ...
                             */
                            if (!this.partExchangeValuationIsValid || !this.PxVuex.Valuation.valuationCompleted) {
                             ...
                             */
                            if (!this.hasPXResult || !this.PxVuex.Valuation.valuationCompleted) {
                                this.PartExchangeFilter.stepTwoSubStep = 0;
                            }
                        }
                        this.currentStep = 1;
            ...
            updateCurrentStep(newCurrentStep = false) {
                ...
                switch (currentStep) {
                    ...
                    case 1:
                        ...
                        jQuery(window).trigger('scroll'); // on some steps, after changing the content, some UI
                        // elements won't rearrange themselves - they need to nudged with this event
                        break;
                    case 2:
                        // Model filter
        ...
        ready() {
            ...
            if (!this.FinanceFilter.financeParams.acceptedFinance) {
                ...
                switch (parseInt(this.currentSavedStep)) {
