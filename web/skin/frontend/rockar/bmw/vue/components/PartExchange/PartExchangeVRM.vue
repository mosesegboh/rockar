<template>
    <div :class="['px-vrm-block', isPopup ? 'inside-popup' : '', isAccordion ? 'inside-accordion' : '']">
        ...
        <div class="px-vrm-block-input" v-else>
            <app-select title="-- Select your car --"
                        :item-height="79"
                        :options="carAlternatives"
                        custom-event="PartExchangeVRM::setAlternativeCar">
            </app-select>
        </div>
        ...
        <div class="px-car-info" v-if="PX.canOpenCustom && carNotFound">
            <p class="px-error">
                ...
                <a href class="px-link" @click.prevent="openCustom">{{ 'click here' | translate }}</a>
        ...
        <div class="px-car-info" v-if="!PX.canOpenCustom && carNotFound">
            <p class="px-error">
                {{ 'Hmmmm. Sorry, we couldn\'t find your vehicle' | translate }}
        </div>

        <div class="px-car-info" v-if="plateNumberError">
            <p class="px-error">
                {{ 'Hmmmm. Looks like you\'ve entered wrong number' | translate }}
            </p>
        </div>
            ...
            <div class="px-mileage-block-wrapper">
                ...
                <div class="px-vrm-block-input" v-show="!carAlternatives.length || carDetails.result">
                    <input
                        ...
                        :disabled="valuationResult || carDetails.vrmDisabled"
                    >
                </div>
            </div>
        </div>
