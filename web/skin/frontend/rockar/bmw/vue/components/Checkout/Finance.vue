<template>
    ...
    <div class="reseller-modal-wrapper">
        <app-modal  width="80%" :show.sync="reseller" class-name="simple-popup checkout-finance-confirmation-popup">
            <div :show.sync="!resellerDisagree" slot="content">
                <div class="valuation-result-continue row">
                    <div class="valuation-result-continue-block">
                        {{{ resellerOverlayContent | translate }}}
                        <div class="row align-right">
                            <button class="row-element button button-confirm" @click="submit()" :disabled="submitDisabled">{{ 'Agree' | translate }}</button>
                            <button class="row-element button button-gray-light" @click="selectDisagreeAndContinue()">{{ 'Disagree' | translate }}</button>
                        </div>
                    </div>
                </div>
            </div>
        </app-modal>
        <app-modal :show.sync="resellerDisagree" width="80%">
            <div slot="content">
                {{{ resellerDisagreeOverlayContent | translate }}}
            </div>
        </app-modal>
    </div>
...
<script>
    ...
    export default Vue.extend({
        ...
        computed: {
            ...
            isValid() {
                return this.$summary.terms.modified && this.$summary.card.modified && this.$summary.valid;
            },
        ...
        methods: {
            submit() {
                ...
                /* Card is selected validation */
                if (this.chosenCard === '' || !this.chosenCard) {
                    this.cardError = true;
                    return;
                }
