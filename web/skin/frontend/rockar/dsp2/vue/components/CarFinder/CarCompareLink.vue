        ...
        <div class="compare-link-content">
            <template v-if="currentDevice !== 'mobile'">
                <app-tooltip :tooltip-position="'top-left'" :tooltip-width="400">
                    <a @click="openCompare" href="javascript:void(0);" class="button button-empty-gray" slot="tooltipElement">{{ 'Compare Cars' | translate }}
            ...
            <template v-else>
                <a @click="openCompare" href="javascript:void(0);" class="button button-empty-gray">{{ 'Compare Cars' | translate }}
                    <span class="badge">{{ compare.carsIn }}</span>
                </a>
            </template>
        </div>
        ...
        <app-modal :show.sync="openWarning" class-name="simple-popup">
            <div slot="content" class="compare-warning">
                <div class="compare-content" v-if="compare.carsIn == 0">
                    <p class="h1">{{ 'No cars in compare list:' | translate }}</p>
                    <p>{{ 'There were no cars selected for car compare' | translate }}</p>
                </div>

                <div class="compare-content" v-if="compare.carsIn == 1">
                    <p class="h1">{{ 'More cars needed for compare:' | translate }}</p>
                    <p>{{ 'There was only 1 car selected for car compare. Please select at least 1 other vehicle for comparison.' | translate }}</p>
                </div>
            </div>
        </app-modal>
    </div>
