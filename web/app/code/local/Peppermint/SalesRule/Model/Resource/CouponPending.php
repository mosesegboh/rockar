...
class Peppermint_SalesRule_Model_Resource_CouponPending extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
    ...
    {
        $this->_init('peppermint_salesrule/coupon_pending', 'coupon_id');
        $this->addUniqueField([
            'field' => 'code',
    ...
    public function updateSpecificCoupons(Mage_SalesRule_Model_Rule $rule)
    {
        ...
        if (!empty($updateArray)) {
            $this->_getWriteAdapter()->update(
                $this->getTable('peppermint_salesrule/coupon_pending'),
                $updateArray,
                ['rule_id = ?' => $rule->getId()]
            );
        }
